...
export async function createSlitherClient(context: ExtensionContext): Promise<LanguageClient> {
  ...
  const serverBinary = context.asAbsolutePath(
    path.join('dist', os.platform().startsWith('win') ? 'slither-server.exe' : 'slither-server'),
  );
  ...
  const client = new LanguageClient('osmium-slither', 'Osmium Slither Language Server', serverOptions, clientOptions);
